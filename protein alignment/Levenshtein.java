...
public class Levenshtein {
	...
	*/
	   ...
	   System.out.println("Levenshtein Number is: "+getLevenshteinDistance(X,Y));
	   System.out.println();
	
    }

    public static int getLevenshteinDistance(String sequenceX, String sequenceY) {
	      
	      int xLength = sequenceX.length(); 
	      int yLength = sequenceY.length(); 

	      if (xLength == 0) { return yLength;} 
	      else if (yLength == 0) { return xLength; }

	      if (xLength > yLength) {
	          String tmp = sequenceX;
	          sequenceX = sequenceY;
	          sequenceY = tmp;
	          xLength = yLength;
	          yLength = sequenceY.length();
	      }

	      int p[] = new int[xLength+1]; 
	      int d[] = new int[xLength+1]; 
	      int _d[]; int i; int j; 
	      ...
	      for (j = 1; j<=yLength; j++) {
	          t_j = sequenceY.charAt(j-1);
	          d[0] = j;
	          for (i=1; i<=xLength; i++) {
	              cost = sequenceX.charAt(i-1)==t_j ? 0 : 1;
